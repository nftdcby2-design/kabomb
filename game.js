...
class OptimizedAssetLoader {
	...
	async loadCriticalAssets(onProgress) {
		...
		const timeoutPromise = new Promise((resolve) => {
			setTimeout(() => {
				console.log('⚠️ Loading timeout reached, using fallbacks!');
	...
	async performCriticalLoading(onProgress, addProgress, criticalSteps) {
		try {
			// Load minimal player sprites (only first frame of each essential animation)
			const playerManifest = this.getPlayerManifest();
			if (!playerManifest || !playerManifest._base) {
				console.warn('⚠️ Player manifest missing, using fallbacks');
				this.createFallbackSprites();
				if (onProgress) onProgress(criticalSteps, criticalSteps);
				return this.assets;
			}
			...
			addProgress();
				...
				// Ensure we have minimal required sprites
	...
	// Ensure we have minimal required sprites for the game to function
	ensureMinimalSprites() {
		console.log('🔍 Verifying minimal sprites are available...');
		
		// Required player animations
		const requiredPlayerAnims = ['1-Idle', '2-Run', '4-Jump', '5-Fall'];
		for (const anim of requiredPlayerAnims) {
			if (!this.assets.player[anim] || this.assets.player[anim].length === 0) {
				console.log(`⚠️ Missing ${anim}, creating fallback...`);
				this.assets.player[anim] = [this.createSimpleFallback('#228B22', 'P')];
			}
		}
		
		// Ensure we have at least one enemy type
		if (!this.assets.enemies['Bald Pirate'] || !this.assets.enemies['Bald Pirate']['1-Idle']) {
			this.assets.enemies['Bald Pirate'] = { '1-Idle': [this.createSimpleFallback('#DC143C', 'E')] };
		}
		
		// Ensure we have basic objects
		if (!this.assets.objects['1-BOMB'] || !this.assets.objects['1-BOMB']['1-Bomb Off']) {
			this.assets.objects['1-BOMB'] = { '1-Bomb Off': [this.createSimpleFallback('#2F2F2F', 'B')] };
		}
		
		if (!this.assets.objects.tiles || !this.assets.objects.tiles.blocks) {
			this.assets.objects.tiles = { blocks: [this.createSimpleFallback('#8B4513', 'T')] };
		}
		
		console.log('✅ Minimal sprites verified!');
	}
	
	createSimpleFallback(color, letter) {
